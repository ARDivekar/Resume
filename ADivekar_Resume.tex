...
\descript{Machine Learning Scientist - 2}
\runsingleline{(Oct 2020 \textendash{} Present, India)}
\begin{tightemize}
    \item Developed AutoML framework ``DRiP'' to iteratively maximize AutoML performance while restricted by a user-defined cost budget. Paper accepted at Amazon ML Conference 2021 (33\% acceptance-rate out of $\sim$750 submissions).
    \item Proposed ``GROK'' score to rank the quality of text augmented by Machine-translation and Abstractive Summarization models (BART, T5, PEGASUS). On product-classification tasks, GROK requires $\sim$70\% fewer samples to achieve performance of Beam-Search and rule-based augmentation.
\end{tightemize}
\sectionsep

\runsubsection{Amazon}
\descript{Machine Learning Scientist - 1}
...
\descript{Research Assistant} \runsingleline{(Apr \textendash{} Jun 2017, India)}
\begin{tightemize}
...
\runsingleline{2020 - 2023 (Expected) \textbullet{} GPA: 4.0} \\
Graduate coursework:
\begin{tightemize}
...
\runsubsection{Conference publications} 
\begin{tightemize}
...
\pagebreak
\runsubsection{Workshop publications}
\begin{tightemize}
